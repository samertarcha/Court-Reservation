...
ajax.open("GET", "../modules/data/reservationTableDataBasketball.php", true);
...
ajax.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
        ...
        // DOM elements
        const basketball = document.getElementById("basketballNextAvailableSlot");
        ...
        if (data.length == 0) {
            if (dayOfWeekJS == 'Sun' && time < startSun) {
                basketball.innerHTML = `${displayToday} | ${displayStartSun}`;
            }
            else if (dayOfWeekJS == 'Sun' && time > startSun && time < endSun) {
                basketball.innerHTML = `${displayToday} | ${tConvert(time)}`;
            }
            else if (dayOfWeekJS == 'Sun' && time > endSun) {
                basketball.innerHTML = `${displayTomorrow} | ${displayStartMonThu}`;
            }
            else if ((dayOfWeekJS == "Mon" || dayOfWeekJS == "Tue" || dayOfWeekJS == "Wed" || dayOfWeekJS == "Thu") && time <= startMonThu) {
                basketball.innerHTML = `${displayToday} | ${displayStartMonThu}`;
            }
                ...
                if (time > endMonThuMinus30) {
                    basketball.innerHTML = displayTomorrow;
                    breakLoop = true;
                } else {
                    basketball.innerHTML = `${displayToday} | ${tConvert(time)}`;
                }
                ...
                basketball.innerHTML = `${displayTomorrow} | ${displayStartFri}`;
            }
            else if (dayOfWeekJS == "Fri" && time <= startFri) {
                basketball.innerHTML = `${displayToday} | ${displayStartFri}`;
            }
            else if (dayOfWeekJS == "Fri" && time > startFri && time < endFri) {
                basketball.innerHTML = `${displayTomorrow} | ${tConvert(time)}`;
            }
            else if (dayOfWeekJS == "Fri" && time > endFri) {
                basketball.innerHTML = `${displayTomorrow} | ${displayStartSat}`;
            }
            else if (dayOfWeekJS == "Sat" && time <= startSat) {
                basketball.innerHTML = `${displayToday} | ${displayStartSat}`;
            }
            else if (dayOfWeekJS == "Sat" && time > startSat && time < endSat) {
                basketball.innerHTML = `${displayToday} | ${tConvert(time)}`;
            }
            else if (dayOfWeekJS == "Sat" && time > endSat) {
                basketball.innerHTML = `${displayTomorrow} | ${displayStartSun}`;
            }
            else {
                basketball.innerHTML = "err";
            }
                ...
                if (data[0].reservation_date == day) {
                    if (data[0].reservation_start <= startPlus30) {
                        if (data[0].reservation_end <= endMinus30) {
                            basketball.innerHTML = `${displayToday} | ${tConvert(data[0].reservation_end)}`;
                        } else {
                            basketball.innerHTML = `${displayTomorrow} | ${startTomorrow}`;
                        }
                    } else {
                        basketball.innerHTML = `${displayToday} | ${startToday}`;
                    }
                } else {
                    basketball.innerHTML = `${displayToday} | ${startToday}`;;
                }
            ...
            function oneReservationCheckAfter (day, startPlus30, endMinus30, startToday, startTomorrow) {
                if (data[0].reservation_date == day) {
                    if (data[0].reservation_start <= startPlus30) {
                        if (data[0].reservation_end <= endMinus30) {
                            basketball.innerHTML = `${displayTomorrow} | ${tConvert(data[0].reservation_end)}`;
                        } else {
                            basketball.innerHTML = `${displayAfterTomorrow} | ${startTomorrow}`;
                        }
                    } else {
                        basketball.innerHTML = `${displayTomorrow} | ${startToday}`;
                    }
                } else {
                    basketball.innerHTML = `${displayTomorrow} | ${startToday}`;;
                }
            ...
            for (var i = 0; i < data.length; i++) {
                    ...
                    if (data[i].reservation_date == date && data[i].reservation_start <= time && data[i].reservation_end >= time) {
                        if (gapAfterCurrentReservation > 30) {
                            if (data[i].reservation_end <= endMinus30) {
                                basketball.innerHTML = `${setDateFromDatabase(CurrentReservation)}  ${tConvert(data[i].reservation_end)}`;
                                breakLoop = true;
                            } else {
                                basketball.innerHTML = displayTomorrow;
                                breakLoop = true;
                            ...
                            basketball.innerHTML = `${setDateFromDatabase(CurrentReservation)}  ${tConvert(data[i + 1].reservation_end)}`;
                        }
                ...
                function multiReservationCheckAfter(day, startPlus30, endMinus30, startToday) {
                    if (data[i].reservation_date == day) {
                        if (data[i].reservation_start > startPlus30 && i == 0) {
                            ...
                            basketball.innerHTML = displayAfterTomorrow;
                            breakLoop = true;
                        } else if (gapAfterCurrentReservation <= 30) {
                            basketball.innerHTML = `${setDateFromDatabase(CurrentReservation)}  ${tConvert(data[i + 1].reservation_end)}`;
                        } else {
                ...
                // Shouldn't be executed unless one of the slots is not mentioned in a if statement above
                    ...
                    document.getElementById('basketballNextAvailableSlot').innerHTML = "SomTing Wong end";
                    document.getElementById('basketballNextAvailableSlot').innerHTML = "SomTing Wong end";
                    document.getElementById('basketballNextAvailableSlot').innerHTML = "SomTing Wong end";
                }
