...
ajax.open("GET", "../modules/data/reservationTableDataFootball.php", true);
...
ajax.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
        ...
        // DOM elements
        const football = document.getElementById("footballNextAvailableSlot");
        ...
        if (data.length == 0) {
            if (dayOfWeekJS == 'Sun' && time < startSun) {
                football.innerHTML = `${displayToday} | ${displayStartSun}`;
            }
            else if (dayOfWeekJS == 'Sun' && time > startSun && time < endSun) {
                football.innerHTML = `${displayToday} | ${tConvert(time)}`;
            }
            else if (dayOfWeekJS == 'Sun' && time > endSun) {
                football.innerHTML = `${displayTomorrow} | ${displayStartMonThu}`;
            }
            else if ((dayOfWeekJS == "Mon" || dayOfWeekJS == "Tue" || dayOfWeekJS == "Wed" || dayOfWeekJS == "Thu") && time <= startMonThu) {
                football.innerHTML = `${displayToday} | ${displayStartMonThu}`;
            }
                ...
                if (time > endMonThuMinus30) {
                    football.innerHTML = displayTomorrow;
                    breakLoop = true;
                } else {
                    football.innerHTML = `${displayToday} | ${tConvert(time)}`;
                }
                ...
                football.innerHTML = `${displayTomorrow} | ${displayStartFri}`;
            }
            else if (dayOfWeekJS == "Fri" && time <= startFri) {
                football.innerHTML = `${displayToday} | ${displayStartFri}`;
            }
            else if (dayOfWeekJS == "Fri" && time > startFri && time < endFri) {
                football.innerHTML = `${displayTomorrow} | ${tConvert(time)}`;
            }
            else if (dayOfWeekJS == "Fri" && time > endFri) {
                football.innerHTML = `${displayTomorrow} | ${displayStartSat}`;
            }
            else if (dayOfWeekJS == "Sat" && time <= startSat) {
                football.innerHTML = `${displayToday} | ${displayStartSat}`;
            }
            else if (dayOfWeekJS == "Sat" && time > startSat && time < endSat) {
                football.innerHTML = `${displayToday} | ${tConvert(time)}`;
            }
            else if (dayOfWeekJS == "Sat" && time > endSat) {
                football.innerHTML = `${displayTomorrow} | ${displayStartSun}`;
            }
            else {
                football.innerHTML = "err";
            }
                ...
                if (data[0].reservation_date == day) {
                    if (data[0].reservation_start <= startPlus30) {
                        if (data[0].reservation_end <= endMinus30) {
                            football.innerHTML = `${displayToday} | ${tConvert(data[0].reservation_end)}`;
                        } else {
                            football.innerHTML = `${displayTomorrow} | ${startTomorrow}`;
                        }
                    } else {
                        football.innerHTML = `${displayToday} | ${startToday}`;
                    }
                } else {
                    football.innerHTML = `${displayToday} | ${startToday}`;;
                }
            ...
            function oneReservationCheckAfter (day, startPlus30, endMinus30, startToday, startTomorrow) {
                if (data[0].reservation_date == day) {
                    if (data[0].reservation_start <= startPlus30) {
                        if (data[0].reservation_end <= endMinus30) {
                            football.innerHTML = `${displayTomorrow} | ${tConvert(data[0].reservation_end)}`;
                        } else {
                            football.innerHTML = `${displayAfterTomorrow} | ${startTomorrow}`;
                        }
                    } else {
                        football.innerHTML = `${displayTomorrow} | ${startToday}`;
                    }
                } else {
                    football.innerHTML = `${displayTomorrow} | ${startToday}`;;
                }
            ...
            for (var i = 0; i < data.length; i++) {
                    ...
                    if (data[i].reservation_date == date && data[i].reservation_start <= time && data[i].reservation_end >= time) {
                        if (gapAfterCurrentReservation > 30) {
                            if (data[i].reservation_end <= endMinus30) {
                                football.innerHTML = `${setDateFromDatabase(CurrentReservation)}  ${tConvert(data[i].reservation_end)}`;
                                breakLoop = true;
                            } else {
                                football.innerHTML = displayTomorrow;
                                breakLoop = true;
                            ...
                            football.innerHTML = `${setDateFromDatabase(CurrentReservation)}  ${tConvert(data[i + 1].reservation_end)}`;
                        }
                ...
                function multiReservationCheckAfter(day, startPlus30, endMinus30, startToday) {
                    if (data[i].reservation_date == day) {
                        if (data[i].reservation_start > startPlus30 && i == 0) {
                            ...
                            football.innerHTML = displayAfterTomorrow;
                            breakLoop = true;
                        } else if (gapAfterCurrentReservation <= 30) {
                            football.innerHTML = `${setDateFromDatabase(CurrentReservation)}  ${tConvert(data[i + 1].reservation_end)}`;
                        } else {
                ...
                // Shouldn't be executed unless one of the slots is not mentioned in a if statement above
                    ...
                    document.getElementById('footballNextAvailableSlot').innerHTML = "SomTing Wong end";
                    document.getElementById('footballNextAvailableSlot').innerHTML = "SomTing Wong end";
                    document.getElementById('footballNextAvailableSlot').innerHTML = "SomTing Wong end";
                }
